...
const datosTalleres: any = {
  AGOSTO: [
    { id: 1, dia: 6, nombre: 'SÁBADO', titulo: 'Pinta tu taza en cerámica', color: 'bg-[#a366ab]', hex: '#a366ab', hora: '6:00 PM - 8:00 PM', instructor: 'ROCIO CAMPA', slug: 'rocio-campa', descripcion: 'PIEZA DE CERÁMICA, GUÍA PASO A PASO, PINCELES, PIGMENTOS ILIMITADOS, QUEMA FINAL DE LA PIEZA, SNACKS Y BEBIDAS.' },
    { id: 2, dia: 13, nombre: 'SÁBADO', titulo: 'Pinta tu regalo para papá', color: 'bg-[#a366ab]', hex: '#a366ab', hora: '6:00 PM - 8:00 PM', instructor: 'ELENA GARZA', slug: 'elena-garza', descripcion: 'MATERIALES INCLUIDOS PARA CREAR UN REGALO ÚNICO Y ESPECIAL UTILIZANDO TÉCNICAS MIXTAS.' },
    { id: 3, dia: 18, nombre: 'JUEVES', titulo: 'Teje tu portavasos en crochet', color: 'bg-[#f29278]', hex: '#f29278', hora: '5:00 PM - 7:00 PM', instructor: 'MARÍA LÓPEZ', slug: 'maria-lopez', descripcion: 'APRENDE PUNTOS BÁSICOS DE TEJIDO Y LLÉVATE TU SET DE PORTAVASOS ARTESANALES.' },
    { id: 4, dia: 28, nombre: 'DOMINGO', titulo: 'Introducción al dibujo', color: 'bg-[#db5c8d]', hex: '#db5c8d', hora: '10:00 AM - 1:00 PM', instructor: 'CARLOS RUIZ', slug: 'carlos-ruiz', descripcion: 'DOMINA LAS LUCES Y SOMBRAS CON CARBONCILLO EN ESTE TALLER PARA PRINCIPIANTES.' },
  ],
  SEPTIEMBRE: [
    { id: 5, dia: 12, nombre: 'VIERNES', titulo: 'Fotografía de Paisaje Urbano', color: 'bg-[#5ca9e6]', hex: '#5ca9e6', hora: '4:00 PM - 7:00 PM', instructor: 'JOSE VALLEJO', slug: 'jose-vallejo', descripcion: 'DOMINA LA COMPOSICIÓN Y LA LUZ NATURAL EN ENTORNOS URBANOS. INCLUYE RECORRIDO Y EDICIÓN BÁSICA.' },
  ]
};
    ...
    <main className="min-h-screen pt-32 pb-20 bg-[#e2e4e7] px-8">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center">
                ...
                >
                  <div className="flex justify-between items-start">
                    <span className="uppercase text-[10px] font-black tracking-tighter opacity-70">
                      {dia % 7 === 1 ? 'Lun' : dia % 7 === 2 ? 'Mar' : dia % 7 === 3 ? 'Mié' : dia % 7 === 4 ? 'Jue' : dia % 7 === 5 ? 'Vie' : dia % 7 === 6 ? 'Sáb' : 'Dom'}
                    </span>
                    <span className="text-2xl font-black">{dia < 10 ? `0${dia}` : dia}</span>
                  </div>
                  {taller && (
                    <div className="mt-auto">
                      <p className="text-[10px] font-bold leading-tight line-clamp-2 italic mb-1 uppercase">{taller.titulo}</p>
                      <span className="bg-black/20 text-[8px] uppercase font-bold py-0.5 px-2 rounded w-fit">Taller</span>
                    </div>
                  )}
                </div>
              );
          ...
          {/* LADO DERECHO: DETALLES CENTRADOS */}
          <div className="w-full lg:w-[40%] flex items-center justify-center min-h-[450px]"> 
            {tallerSeleccionado ? (
                ...
                <div className="space-y-6 text-[#2d2d44] font-bold tracking-[0.15em] uppercase text-sm">
                  ...
                  <div className="pt-6 border-t border-gray-400">
                    <p className="text-xs">
                      IMPARTIDO POR: 
                      <Link 
                        href={`/artistas/${tallerSeleccionado.slug}`} 
                        className="underline decoration-2 underline-offset-4 cursor-pointer hover:opacity-60 ml-2"
                      >
                        {tallerSeleccionado.instructor}
                      </Link>
                    </p>
                  </div>
                </div>
              ...
              <div className="text-[#2d2d44] text-center lg:text-left">
                ...
                <p className="text-sm font-bold opacity-60 leading-relaxed uppercase tracking-[0.2em]">
                  Explora nuestras experiencias creativas de {mesNombre}. Selecciona un día marcado para ver los detalles y conocer al artista.
                </p>
              </div>
            )}
          </div>
